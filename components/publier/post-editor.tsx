...
import { Sparkles, ImagePlus, AlertTriangle, CheckCircle2, Send, CalendarClock } from "lucide-react";

const CONNECTED = new Set<NetworkId>(
  sampleSocialAccounts.filter((a) => a.status === "connecté").map((a) => a.network),
);
...
export function PostEditor() {
  const [baseCaption, setBaseCaption] = useState(
    "3 signes que votre offre n'est pas claire — et comment les corriger cette semaine.",
  );
  const [selected, setSelected] = useState<Set<NetworkId>>(new Set(["instagram", "facebook"]));
  const [captions, setCaptions] = useState<Partial<Record<NetworkId, string>>>({});
  const [activeTab, setActiveTab] = useState<NetworkId>("instagram");
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
      <div className="flex flex-col gap-4">
        <Card>
          ...
          <button
            type="button"
            className="mb-4 flex h-28 w-full items-center justify-center gap-2 rounded-xl border border-dashed border-line text-sm text-ink-secondary hover:border-emerald/40 hover:text-ink"
          >
        ...
        {selectedList.length > 0 ? (
          <Card>
            ...
            {selectedList.includes(activeTab) ? (
              <div className="flex flex-col gap-4">
                <Field
                ...
                {activeTab === "youtube" ? (
                  <Field label="Titre YouTube">
                    <Input placeholder="3 signes que votre offre n'est pas claire" />
                  </Field>
                ) : null}
                {activeTab === "tiktok" ? (
